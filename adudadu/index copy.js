...
function chanceChange(t, x, y) {
    ...
    peluang.innerHTML = `<span>${count}</span> <span></span><span>36</span>`;
    // yellow.forEach(item => {
    //     item.innerHTML = `${x} , ${y}`
    // })
}
...
function indexing() {
    ...
    if (counter > 3) {
        ...
        let indexStars = 0;
        console.log(stars)
        document.querySelector('.inner').innerHTML = `<h4 class="habis"><span>Selamat! Kamu telah berhasil memahami ruang sampel dengan baik!</span><br/><span id="rate" style="font-size:450%">
        ${Array(5).fill(0).map(e => indexStars++ < stars ? '&#9733;' : '&#9734;').join('')}
        </span><br/><span>Total jawaban benar ${cleanBenar} dari ${totalBenar}</span></h4 > <button class="reset" onclick="reset()" > Ulangi</button >`;

        localStorage.setItem('gameStart', false);
        return;
    }
    document.querySelectorAll('.wrapper-left button')[1].style.display = 'none';
    document.querySelectorAll('.wrapper-left button')[0].style.display = 'block';
    ...
    document.getElementById('benar').innerHTML = `<span>?? dari ??</span>`;
    document.getElementById('lewat').innerHTML = `<span>??</span>`;
    document.getElementById('salah').innerHTML = `??`;
    peluang.innerHTML = `<span>0</span> <span></span><span>36</span>`;
...
function checkAnswer() {
    ...
    document.getElementById('benar').innerHTML = `<span>${answer.benar} dari ${answer.all}</span>`;
    document.getElementById('lewat').innerHTML = `<span>${answer.all - answer.benar}</span>`;
    document.getElementById('salah').innerHTML = `${answer.salah}`;

    totalBenar += answer.all;
    cleanBenar += answer.benar;
    peluang.innerHTML = `<span>${answer.all}</span> <span></span><span>36</span>`;
    ...
    document.querySelectorAll('.wrapper-left button')[0].style.display = 'none';
    document.querySelectorAll('.wrapper-left button')[1].style.display = 'block';
}
